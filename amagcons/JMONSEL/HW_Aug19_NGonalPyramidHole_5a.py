...
def makeConePlanes(Ndivisions,radius,apex,min=1e-15):
...
if (WinOrLin == 1): 
	tablePathOuter = "C:\Program Files\NIST\JMONSEL\ScatteringTables"       			   # for home/workPC, "C:\NIST\JMONSEL\ScatteringTables"
if (WinOrLin == 2): 
...
layer1thicknessnm = 200.	# Thickness in nm of the 1st layer (immediately below the lines)
layer2thicknessnm = 50.# Thickness in nm of the 2nd layer
...
print >>file, "beamsize(nm)  h(nm)     w(nm)  beamE(eV)   Experiment#	x(nm)	 y(nm)	BSEyield	SEyield	  ElapsedTime"     #print >>file, "beamsize(nm) h(nm) w(nm) SWAl(deg) SWAr(deg) beamE (eV)	x(nm)	y (nm)	BSE yield	SE yield	Elapsed Time"
print         "beamsize(nm)  h(nm)     w(nm)  beamE(eV)   Experiment#	x(nm)	 y(nm)	BSEyield	SEyield	  ElapsedTime"     #print "beamsize(nm) h(nm) w(nm) SWAl(deg) SWAr(deg) beamE (eV)	x(nm)	y (nm)	BSE yield	SE yield	Elapsed Time"
...
for beamsizenm in beamsizenmvals:
	...
	layer1Region = monte.addSubRegion(chamber,CuMSM,layer1)  
	...
	layer2Region = monte.addSubRegion(layer1Region,SiO2MSMDeep,layer2)  
	...
	for hnm in hnmvals:
		...
		for wnm in wnmvals:
			...
			for SWAtemp in SWAvals:
                ...
                #########New SPIE2020 cell#########
				#######tall   h=50nm########
                block = mon.NShapes.createLine(-CTheight*1E-9,Wbox*1E-9,Lbox*1E-9,0.,0.,0.,0.)  #	line = mon.NShapes.createLine(-h,w,linelength,thetal,thetar,radl,radr)
                ...
                conePlanes = makeConePlanes(NConeDivisions,CTdiameter/2*1E-9,[0.,0.,vertexZ*1E-9])
                print(conePlanes)
                for plane in conePlanes:
                    pyramid.addPlane(*plane)                
                pyramid.addPlane([0.,0.,-1.],[0.,0.,0.]) #plane pointing up thru origin for base of pyramid
                pyramid.addPlane([0,0,1],[0,0,CTheight*1E-9]) #cutoff planes
                pyramid.translate([+0.*1E-9,0.,-CTheight*1E-9])

                DiffShape=mon.NormalDifferenceShape(block,pyramid)
                DiffShapeRegion = monte.addSubRegion(chamber,SiO2MSM,DiffShape)
                    ...
                    for m5 in m5vals:
                        for xnm in xvals:          #####################To switch x and y, do it here and in 4 lines below
                            ...
                            for ynm in yvals:
                                ...
                                print >>file, "%8.2f \t%8.2f \t%8.2f \t%8.1f \t%8.1f \t%8.2f \t%8.2f \t%3.5f \t%3.5f \t%8.4f" % (beamsizenm,hnm,wnm,beamEeV,m5,xnm,ynm,bsf,SEf,t)
                                print "%8.2f \t%8.2f \t%8.2f \t%8.1f \t%8.1f \t%8.2f \t%8.2f \t%3.5f \t%3.5f \t%8.4f" % (beamsizenm,hnm,wnm,beamEeV,m5,xnm,ynm,bsf,SEf,t)             #print "%8.2f \t%8.2f \t%8.2f \t%8.2f \t%8.2f \t%8.1f \t%8.2f \t%8.2f \t%3.4f \t%3.4f \t%8.4f" % (beamsizenm,hnm,wnm,thetaldeg,thetardeg,beamEeV,xnm,ynm,bsf,SEf,t)
