...
#ask user for frequency of recreating geometry

#print ('Redo geometry after how many pixels? ')
#pixels_geometry=sys.stdin.readline()
pixels_geometry=1
...
#fos=jio.FileOutputStream("%s/angle - %lg deg.wrl" % (dest, phi))
	...
	if trajLog:  # add a trajectory log listener
        ...
        tfirst = (jl.System.currentTimeMillis()-t0)/1000.

        if pixelnum%pixels_geometry==0:
            monte,eg,detectorRegionList=geometry.definesample(beamsizenm*meterspernm,paramlist[0])
...
# Define our backscatter detector.
	...
	t = (jl.System.currentTimeMillis()-t0)/1000.
	rdirname = jl.Integer(int(beamEeV)).toString()+"eV_"+"x"+jl.Integer(int(xnm)).toString()+'y'+jl.Integer(int(ynm)).toString()
        ...
        rotbeam="x0_y0_z0"
        energydep="NA"
        savedata.writeChamberData(fileid,pixelnum,imagenum,t,t-tbefore,xnm,ynm,mexp,nTrajectories,beamEeV,beamsize,beamsize,rotbeam,energydep,histfile,trajImgFile,trajlogfile,bsf,SEf,Xfchamber,paramlist)
        tprevsec=t*3600
...
#	print >>file, "%8.2f \t%8.1f \t%8.1f \t%8.2f \t%8.2f \t%3.5f \t%3.5f \t%8.4f \t%8.2f" % (beamsizenm,beamEeV,mexp,xnm,ynm,bsf,SEf,t,paramlist[0])
	print "%12.2f \t%6.3f \t%8.1f \t%8.0f \t%8.2f \t%8.2f \t%3.5f \t%3.5f \t%3.5f \t%3.5f \t%12.6f \t%3.3f \t%8.2f" % (pixelnum,beamsizenm,beamEeV,mexp,xnm,ynm,bsf,SEf,detBsf,detBSf2,t,100.*pixelnum/totalpixels,paramlist[0])   #modified by BB
...
#   print >>file, "beamsize(nm)	beamE(eV)   Experiment#	x(nm)	 y(nm)	BSEyield	SEyield	det BSf	detectedFraction	ElapsedTime	variable 1"  
   ...
   if "xyvals" in scanregion.keys():
       ...
       scanregioninfo['pixels(ny_ny)'] = ' '+str(len(xyvals))
       ...
       scanregioninfo['pixels(ny_ny)'] = ' '+str(len(xvals))+', '+str(len(yvals))
       scanregioninfo['start(x_y)'] = ' '+str(xvals[0])+', '+str(yvals[0])
...
# create scanregioninfo dictionary with info for header
   ...
   print         "       Pixel        beamX        beamE      Frame     x(nm)      y(nm)  BSEyld	  SEyld   detBSf   detFrac          TimeSec       %comp        var1"    #modified by BB
